...
class App:
    ...
    def start_one_portion(self):
        if self.waage.get_tared_weight() - self.portion_weight <= 0:
            Logger.log_error('Kann nicht starten - zu wenig Gewicht', self.__class__, self.rtc.get_time())
            return False
        Logger.log_info('Start One Portion with ' + str(self.waage.get_tared_weight()), self.__class__, self.rtc.get_time())
    ...
    def _check_weight(self, timer):
        ...
        if self.waage.get_tared_weight() <= self.stop_by_weight:
            self.tim0.deinit()
            Logger.log_info('Stopped by ' + str(self.waage.get_tared_weight()), self.__class__, self.rtc.get_time())
            self.stop()
